  ...
  try {
    ...
    if (user.orders.length > 0) {
      for (let j = 0; j < user.orders.length; j++) {
        let cartSchema = [];
        console.log(user.orders[j].items.length);
        for (let i = 0; i < user.orders[j].items.length; i++) {
          const cartItem = user.orders[j].items[i];
          const product = await Product.findById(cartItem.product);
          user.orders[j].items[i].product = product;
        }
      }
    }
